...
p2min ps = map h ps
 where
    h p = filter (> 0) $ map (\v -> isDifferentG p v) $ variations p

    g p = case filter (> 0) $ map (\v -> isDifferentG p v) $ variations p of
        [] -> error $ show (p, rline p, (map rline (variations p)))
        (h:_) -> h
    isDifferentG p p' =
        let c = (mcol p)
            r = (mrow p)
            c' = (mcol p')
            r' = (mrow p')
        in if c' == 0 && r' == 0 then 0 -- no reflections
           else rline p'
        --  > 0 && c' /= c then c'
        --    else if r' > 0 && r' /= r then r' * 100
        --    else 0
        -- in if c' == 0 && r' == 0 then 0
        --    else if c == 0 && r == 0 then error (show p)
        --    else if c' /= c then c'
        --    else if r' /= r then r' * 100
        --    else 0
...
variations p = catMaybes [opposite p y x | y <- [0..length p - 1], x <- [0..length (p !! y) - 1]]
  where
    oc = mcol p
    or = mrow p
    opposite :: Pattern -> Int -> Int -> Maybe Pattern
    opposite p y x = if (y ) == or || (x ) == oc then Nothing else
                     let (up, rest) = splitAt y p
                         ([row], down) = splitAt 1 rest
                         (left, rrow) = splitAt x row
                         ([c], right) = splitAt 1 rrow
                     in Just (up ++ [left ++ ((opp c) : right)] ++ down)
    opp :: Char -> Char
    opp '.' = '#'
    opp '#' = '.'
